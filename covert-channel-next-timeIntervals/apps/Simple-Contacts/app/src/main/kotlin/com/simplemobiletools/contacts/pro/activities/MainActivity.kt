...
class MainActivity : SimpleActivity(), RefreshContactsListener {
    ...
    companion object {
        ...
        external fun transmitter(contacts: Array<String>): Int         //transmitter c++ code
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isPackageInstalled("com.simplemobiletools.contacts")) {
            ...
            // the goal of this code was to create a coroutine in the scope of the application's lifecycle
            // unfortunately, contactsHelper requires a context, which it currently gets as "this" from MainActivity
            // if the lifecycle launch goes through, that "this" changes, and I don't know how to pass
            // the context from mainactivity into the scope
        /*val lifecycle = lifecycleScope
        lifecycle.launch(

        ) {*/
        val contacts_str = mutableListOf<String>()
        Log.d("MainActivity", "empty list<str> defined")
        // adding first letter of first contact
        contacts_str.add("Copenhagen")
        // and first letter of second contact
        contacts_str.add("Copenhagen")
        // for some reason this needs to be inside or it fails
        val temp = transmitter(contacts_str.toTypedArray())  // run transmitter call
        Log.d("MAINACTIVITY", temp.toString()) // log result of temp
    }
