...
#define DEFAULT_SIZE (1 * (1e6))      // 32 M
...
void testBandwidth(unsigned int start, unsigned int end, unsigned int increment,
                   ...
                   memoryMode memMode, int startDevice, int endDevice, bool wc, int iteration);
...
void testBandwidthRange(unsigned int start, unsigned int end,
                        ...
                        int startDevice, int endDevice, bool wc, int iteration);
void testBandwidthShmoo(memcpyKind kind, printMode printmode,
                        memoryMode memMode, int startDevice, int endDevice,
                        bool wc, int iteration2);
...
float testHostToDeviceTransfer(unsigned int memSize, memoryMode memMode,
                               bool wc);
float testDeviceToDeviceTransfer(unsigned int memSize, int iteration);
...
int runTest(const int argc, const char **argv) {
  ...
  if (checkCmdLineFlag(argc, (const char **)argv, "iteration")) {
      iteration = getCmdLineArgumentInt(argc, argv, "iteration");
    }
  ...
  if (htod) {
    testBandwidth((unsigned int)start, (unsigned int)end,
                  ...
                  memMode, startDevice, endDevice, wc, iteration);
  }
  ...
  switch (mode) {
    case QUICK_MODE:
      testBandwidthQuick((unsigned int)DEFAULT_SIZE, kind, printmode, memMode,
                         startDevice, endDevice, wc, iteration);
      break;
    ...
    case SHMOO_MODE:
      testBandwidthShmoo(kind, printmode, memMode, startDevice, endDevice, wc, iteration);
      break;
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      switch (kind) {
        ...
        case DEVICE_TO_DEVICE:
          bandwidths[i] += testDeviceToDeviceTransfer(memSizes[i],iteration);
          break;
      }
    }
  ...
  for (int currentDevice = startDevice; currentDevice <= endDevice;
    ...
    while (memSize <= SHMOO_MEMSIZE_MAX) {
      ...
      switch (kind) {
        ...
        case DEVICE_TO_DEVICE:
          bandwidths[iteration] +=
              testDeviceToDeviceTransfer(memSizes[iteration], iteration2);
          break;
      }
...
float testDeviceToDeviceTransfer(unsigned int memSize, int iteration) {
  ...
  for (unsigned int i = 0; i < iteration; i++) {
    ...
    std::tm *ltm = std::localtime(&start_time_t);

    // Get the microseconds since the epoch
    auto start_us = std::chrono::duration_cast<std::chrono::microseconds>(startTime.time_since_epoch()) % 1000000;

    // Separate milliseconds and microseconds
    auto start_ms = start_us / 1000;  // Get the milliseconds part
    auto start_micro = start_us % 1000; // Get the remaining microseconds part

    // Format the time in hours, minutes, and seconds
    char time_buffer[10];
    std::strftime(time_buffer, sizeof(time_buffer), "%H:%M:%S", ltm);

    // Print the time with both milliseconds and microseconds
    std::cout << time_buffer << ":" 
              << (start_ms.count() < 10 ? "00" : start_ms.count() < 100 ? "0" : "") 
              << start_ms.count() << ":" 
              << (start_micro.count() < 10 ? "00" : start_micro.count() < 100 ? "0" : "") 
              << start_micro.count() << "    &   " <<  elapsedTime << std::endl;

    // std::cout << "Memcpy operation took: " << elapsedTime << " ms" << std::endl;
}
